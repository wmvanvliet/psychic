...
def cross_validate(subsets, node):
    """
    Cross-validate on subsets using node. Returns a list with the output of
    node on the test sets. It can give an estimate of how well the analysis
    will work in practice [1].

    This methods has a few safety measures:
    - for every fold, we start with a fresh copy of node
    - the labels of the test sets are removed to prevent node.test() from
      cheating
    ...
    This applies node n to the train and test sets based on the sequential
    split of dataset d into 5 subsets, and returns the output for each.
    Cross validation is used to see how a certain analysis generalizes from one
    subset of the data to another, without needing an explicit validation
    dataset. 
...
def cross_validation_sets(subsets):
    ...
    Returns a list of training sets and a list of test sets. The first test set
    is the first subset. The first training set are all the subsets except for
    the first subset. The second test set is the second subset, etc.
