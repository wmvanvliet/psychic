...
def lw_cov(X, center=True):
    ...
    For X, a [p x n] matrix, where n is the number of observations, and p is
    the number of variables, a robust covariance estimate is returned. It
    ...
    covariance and the difference between the prior and and the sample
    covariance S.
...
def kl(P, Q):
    ...
    using norm_kl_divergence(). Please note that the inversion of the
    covariance matrix of Q (\Sigma_Q) might cause numerically unstable results
    although lw_cov() is used to estimate \Sigma_Q.

    P and Q are [p x n] matrices, where p is the number of variables, and n is
    the number of observations.

    The Kullback-Leibler divergence is an asymmetric distance measure between
    two probability measures P and Q, measuring the extra information needed to
    repre- sent samples from P when using a code based on Q.
    '''
...
def auc(scores, labels):
    ...
    classifier will rank a randomly chosen positive instanance higher than a
    randomly chosen negative instance [1]. I.e., the AUC is a value between 0
    and 1, and AUC of .5 indicates random ranking performance.
...
def auc_confidence(N, rho=.5, delta=.05):
    '''
    Calculate the confidence interval epsilon for the AUC statistic.  N is the
    number of instances, rho is the percentage of *positive* instances,
