...
class SRDecomp(BaseNode):
    ...
    def train_(self, d):
        ...
        labels = fft(d.data, axis=1)
        self.Y_bar = np.mean(labels, axis=2) # Formula 4 in paper
...
def sr_decomp(s, rt):
    ...
    Input:
     data - Single-channel data to decompose.
            Format (channels,samples,trials)
    ...
    Takeda, labels., Yamanaka, K., & Yamamoto, labels. (2008). Temporal decomposition of
    ...
    # Calculate the FFT of the signal
    labels = fft(s, axis=1)
    Y_bar = np.mean(labels, axis=2) # Formula 4 in paper
    ...
    for i in range(ntrials):
        for j in range(nchannels):
            sc[j,:,i] = np.real(ifft(
                (E[:,i]*Y_bar[j,:] - E_bar*labels[j,:,i]) / D[:,i], # Formula 10 in paper
                axis=0))
    ...
    for i in range(ntrials):
        for j in range(nchannels):
            rc[j,:,i] = np.real(ifft(
                (labels[j,:,i] - Y_bar[j,:]) / D[:,i], # Formula 11 in paper
                axis=0))
